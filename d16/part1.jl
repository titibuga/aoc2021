...
function readPackets(bin_str; num_packets = Inf)
    ...
    while isNonZero(bin_str) && n_packets_read < num_packets
        ...
        if id == 4
            ...
            println("=== OP package")
            (subtotal_v, bin_str) = getOpPacket(bin_str)
            total_ver += subtotal_v
        end
        println("= Reminder: $bin_str")
        n_packets_read += 1
    end
    return total_ver, bin_str
end
...
function getOpPacket(bin_str)
    ...
    if length_type_ID == '0'
        ...
        (total_v, _) = readPackets(contained_bin_str)
    else
        println("Length type: 0")
        num_packets = parseBtoInt( bin_str[1:11] )
        bin_str = bin_str[12:end]
        total_v, bin_str = readPackets(bin_str, num_packets = num_packets) 

    end
    return total_v, bin_str
end
